...
onMounted(async () => {
  try {
    ...
    draft.value = s
    gitlabDraft.value = {
      base_url: s?.gitlab?.base_url || 'https://gitlab.com',
      username: s?.gitlab?.username || '',
  ...
  <div class="h-full flex flex-col">
    <header class="bg-white border-b px-6 py-4 flex items-center justify-between shrink-0">
      <div>
        ...
        <p class="text-sm text-muted-foreground mt-0.5">配置 LLM 和 GitLab 连接</p>
      </div>
      <Button variant="outline" size="sm" @click="emit('back')">返回</Button>
    </header>
